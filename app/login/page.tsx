...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-4 font-sans text-gray-200">
        <Link href="/">
            <img className="absolute left-8 top-10 rotate-90" src="/arrow.png" alt="back" />
        </Link>
      {/* 배경 장식 (image_fe2573.png 스타일 재현) */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-10">
        <svg className="w-full h-full" viewBox="0 0 1000 1000">
          <path d="M0,200 L200,200 L300,400 L700,400 L800,600 L1000,600" stroke="white" fill="none" strokeWidth="1" />
          <path d="M0,800 L300,800 L400,600 L600,600 L700,400 L1000,400" stroke="white" fill="none" strokeWidth="1" />
        </svg>
      </div>
      ...
      <div className="relative w-full max-w-md bg-[#161616] border border-white/10 rounded-2xl p-8 shadow-2xl backdrop-blur-md">
        ...
        {mode !== 'LOGIN' && (
          <button onClick={() => setMode('LOGIN')} className="absolute left-6 top-8 text-gray-500 hover:text-white transition-colors">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-400 mt-2">
            {mode === 'LOGIN' ? (
              <>Don't have an account yet? <button onClick={() => setMode('SIGNUP')} className="text-blue-400 hover:underline font-medium ml-1">Sign up</button></>
            ) : mode === 'SIGNUP' ? (
              <>Already have an account? <button onClick={() => setMode('LOGIN')} className="text-blue-400 hover:underline font-medium ml-1">Login</button></>
            ) : (
        ...
        <form onSubmit={handleEmailAuth} className="space-y-4">
          ...
          >
            {isLoading ? 'Processing...' : mode === 'LOGIN' ? 'Login' : mode === 'SIGNUP' ? 'Continue' : 'Verify'}
          </button>
        </form>
        ...
        {mode === 'LOGIN' && (
          <>
            <div className="flex items-center my-8">
              ...
              <span className="px-4 text-[10px] text-gray-600 font-bold tracking-widest">OR</span>
              <div className="flex-1 h-[1px] bg-white/5" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" className="text-gray-400 group-hover:text-white transition-colors">
              ...
              <button 
                ...
                className="flex items-center justify-center gap-2 bg-[#1e1e1e] border border-white/5 py-3 rounded-xl hover:bg-[#252525] transition-colors group"
              >
                <span className="font-black text-lg text-gray-500 group-hover:text-red-500 transition-colors">G</span>
